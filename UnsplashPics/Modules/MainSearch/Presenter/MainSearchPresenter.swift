...
final class MainSearchPresenterImpl: MainSearchPresenterProtocol {
    ...
    func searchPhotos(with searchTerm: String, filters: String? = nil) async {
        guard searchTerm != lastSearchTerm || filters != lastSearchFilters else { return }
    ...
    func fetchMorePhotos() async {
        ...
        do {
            ...
            case .search:
                guard let newPhotos = try await networkService.fetchMorePhotos(page: page) as? SearchResponse else {
                    return
                }
                hasMore = newPhotos.totalPages > page
                view?.appendNewPhotos(photos: newPhotos.results)
            case .initialLoad:
                guard let newPhotos = try await networkService.fetchMorePhotos(page: page) as? [UnsplashPhoto] else {
                    return
                }
    ...
    private func addToSuggestionsHistory(_ query: String) {
